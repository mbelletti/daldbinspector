...
class DbiSqlite(DbiAdapter):
    ...
    def get_fields(self, table) -> list:
        ...
        return [
            (f['name'], 'id' if f['pk'] == 1 else self.datatypes[f['type'].upper()])
            for f in fields
        ]
...
class DbiPostgresql(DbiAdapter):
    ...
    def get_fields(self, table) -> list:
        ...
        for f in fields:
            if (
                ('column_default' in f)
                and f['column_default']
                and f['column_default'].startswith('nextval')
            ):
...
class DbInspector(object):
    ...
    def table_def(self, table, with_id=False):
        ...
        fields = params['sep'].join(
            ["Field('%s', type='%s')," % (f, t) for f, t in flt_flds]
        )
        if self.schema and self.schema != 'public':
            params.update(
                {'tablename': "%s_%s" % (self.schema, table), 'fields': fields}
            )
        else:
